...
def model_based_controller_dict(robot, contact_state, r_dict, action, lcc_rand, physics_dt = 0.005):
    ...
    pos, ff_torque, info = ft.ftft_step(com_pos, com_vel, r_dict["jacs"],
                             body_pos_w, base_quat,
                             base_angvel, action, contact_state, r_dict["nle"], lcc_rand)
...
class FTFTActionManager(ActionManager):
    @property
    def total_action_dim(self) -> int:
        """Total action dimension."""
        return ft.CTRL_NUM * 2 + 6# + ft.EEF_NUM
...
class FTFEnv(ManagerBasedRLEnv):
    ...
    def step(self, action: torch.Tensor):
        ...
        contact_state = motion.contact_state
